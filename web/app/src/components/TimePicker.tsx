      ...
      <div className="flex items-center justify-between mb-4">
        <div className="w-24 h-16 bg-white rounded-xl flex items-center justify-center border-2 border-primary">
          <input
            ...
            max={12}
            value={selectedHour}
            onChange={(e) => setSelectedHour(`${e.target.value}`.padStart(2, '0'))}
          />
        </div>
        ...
        <div className="w-16  h-16 bg-white rounded-xl flex flex-col text-center">
          <button
            type="button"
            className={`h-1/2 border border-primary text-primary font-bold ${
              selectedPeriod === DayPeriod.AM ? 'bg-secondary' : ''
            } text-primary rounded-t-xl`}
            onClick={() => {
          ...
          <button
            type="button"
            className={`h-1/2 border border-primary text-primary font-bold ${
              selectedPeriod === DayPeriod.PM ? 'bg-secondary' : ''
            } rounded-b-xl`}
            onClick={() => {
        ...
        <div>
          ...
          <button
            ...
            onClick={() =>
              onSubmit(
                `${
                  selectedPeriod === DayPeriod.AM ? selectedHour : Number(selectedHour) + 12
                }:${selectedMinute}`,
              )
            }
          >
